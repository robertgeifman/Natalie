...
static id aspect_add(id self, SEL selector, AspectOptions options, id block, NSError **error) {
    ...
    aspect_performLocked(^{
        if (aspect_isSelectorAllowedAndTrack(self, selector, options, &innerError)) {
            AspectsContainer *aspectContainer = aspect_getContainerForObject(self, selector);
            identifier = [AspectIdentifier identifierWithSelector:selector object:self options:options block:block error:&innerError];
            if (identifier) {
                [aspectContainer addAspect:identifier withOptions:options];

                // Modify the class to allow message interception.
                aspect_prepareClassAndHookSelector(self, selector, &innerError);
            }
        }
    });
    *error = innerError;
...
static BOOL aspect_remove(AspectIdentifier *aspect, NSError **error) {
    ...
    aspect_performLocked(^{
        ...
        if (self) {
            ...
            NSString *errorDescription = [NSString stringWithFormat:@"Unable to deregister hook. Object already deallocated: %@", aspect];

			// AspectError(AspectErrorRemoveObjectAlreadyDeallocated, errorDescription);
			AspectLogError(@"Aspects: %@", errorDescription);
			innerError = [NSError errorWithDomain:AspectErrorDomain code:AspectErrorRemoveObjectAlreadyDeallocated userInfo:@{NSLocalizedDescriptionKey: errorDescription}];
